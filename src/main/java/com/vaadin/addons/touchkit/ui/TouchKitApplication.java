 ...
 * help building Vaadin applications designed specifically for various devices.
 * It provides quick access to {@link WebBrowser} object via
 ...
 * <p>
 * Helper also provides static getter method for TouchKitApplication aka thread
 * local pattern.
 * <p>
 * The standard Vaadin application init process calls {@link #init()} method
 ...
 * developers should override onBrowserDetailsReady() method in case their UI
 * depends on details detected via {@link WebBrowser}. The init method
 * implementation sets a TouchKitWindow as a main window and does the required
 * magic to call {@link #onBrowserDetailsReady()} when JavaScript detected
 * details from the client have been received. On that phase the main window
 * also has window width and heigth set according to the data provided by the
 * client side code.
 * <p>
