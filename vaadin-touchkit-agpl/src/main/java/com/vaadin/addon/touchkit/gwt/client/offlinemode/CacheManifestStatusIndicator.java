...
public class CacheManifestStatusIndicator implements EntryPoint {
	...
	/**
	 * Let the indicator ask the user to reload the application when a new version
	 * of the app has been downloaded.
	 */
	 ...
	 * "updateNowMessage" for specifying the message to show when a new version of
	 * the cache is ready, and "updateCheckInterval" for specifying how often to
	...
	private void loadSettingsFromLocalStorage() {
		...
		if (localStorage != null) {
			...
			String updateCheckIntervalStr = localStorage.getItem(UPDATE_CHECK_INTERVAL_KEY);
			if (updateCheckIntervalStr != null && !updateCheckIntervalStr.isEmpty()) {
	...
	private void scheduleUpdateChecker() {
		Scheduler.get().scheduleFixedPeriod(new Scheduler.RepeatingCommand() {
			...
			public boolean execute() {
				...
				// paused
				if (!isUpdating() && OfflineModeEntrypoint.get().getNetworkStatus().isAppRunning()) {
					updateCache();
				}
				return true;
			}
	...
	/**
	 * Called when a cache event is triggered. All events except for error events
	 * are handled here.
	 * 
	 * @param event
	 *            The event.
	...
	/**
	 * Called when a new version of the application cache (i.e. the widgetset) has
	 * been detected. The default implementation asks the user if we should update
	 * now unless forced.
	 * 
	 * @param force
	 ...
	 */
	@Deprecated
	protected final native void hookAllListeners(CacheManifestStatusIndicator instance)
	/*-{
	}-*/;

	/**
	 * @return The status of the application cache. See the constants in this class
	 *         for possible values. Return 99 if application cache does not exist.
	 */
	...
	/**
	 * Asks the application cache to update itself, i.e. visit the server and check
